...
- **Line-by-line sync (client):** `lib/coding/parser.ts` `ActionAssembler` turns the stream into ordered `Action`s; `useVoiceInterview`'s **action runner** plays them so each code line *types out across the duration of the spoken line that introduces it*, and the next line's narration doesn't start until the current speech finishes. Helpers in `lib/coding/playback.ts` (`ActionQueue`, `estimateSpeechMs`, `tokenizeCode`).
- **Gapless audio (decoupled from visuals):** clauses are **fed to Aura the instant they stream in** (not after the previous clause finishes), so the next clause synthesizes *while* the current one plays and Aura streams them back contiguously — the TTS scheduler appends them with no boundary silence. Narration text + code typing run on a **separate paced chain** (`audioCursor` estimates the spoken timeline) so line-by-line sync is preserved. Feeding eagerly adds **no** up-front latency (the first clause still goes out immediately) — it removes the per-clause synthesis-latency gaps that caused the stutter, without the up-front delay of pre-synthesizing whole lines.
