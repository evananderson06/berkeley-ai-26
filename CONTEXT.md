...
**What it does (met):** the AI candidate speaks its replies and **stops the moment the interviewer's voice crosses a tunable volume threshold**, then listens and replies again. Voice is **always-on** (auto-connects on load — no Start/Pause); the only control is **Mute mic** (see §17.13). **End Interview** marks the candidate completed and attaches a jot-note summary. Everything downstream is unchanged — the transcript stays `Message[]` in localStorage and `/api/interview` + `/api/generate-feedback` are untouched.
...
`new WebSocket('wss://api.deepgram.com/v1/speak?' + params, [DG_AUTH_SCHEME, token])` with `model=aura-2-thalia-en, encoding=linear16, sample_rate=24000`, `binaryType='arraybuffer'`. **Synthesize-whole-line model (fixes stutter + drift):** `synthesize(text)` sends `{Speak}`+`{Flush}` and collects EVERY binary PCM frame until the `Flushed` control frame, concatenating them into ONE `AudioBuffer`; `play(buffer)` plays that single contiguous buffer (so there are no inter-chunk seams/underruns ⇒ no stutter) at `playbackRate = SPEECH_RATE`, and returns the buffer's measured `durationMs` + an `ended` promise. The caller types the line's code over exactly that `durationMs`, so audio and code never drift. `stop()` bumps a generation counter, sends `{Clear}`, cancels any in-flight synthesis (resolves null) and stops the current source (barge-in). Trade-off: a short per-line synthesis delay before audio (acceptable — smoothness over latency).
...
### 17.12 Coding question — speak-while-coding, line-by-line, in-place edits
When the interviewer asks a coding question the turn goes through **`POST /api/interview/code`** (streaming SSE) instead of `/api/interview`. The candidate "thinks out loud while typing":

- **Protocol (server prompt):** the model alternates `[SPEAK]…[/SPEAK]` narration with editor ops — `[CODE]…[/CODE]` (append new code), `[EDIT]old[NEW]new[/EDIT]` (in-place find-and-replace), `[DELETE]old[/DELETE]`, `[CLEAR]` (new problem). It's told to narrate **line by line** — one short spoken thought, then exactly the code that thought describes.
- **Line-by-line sync (client):** `lib/coding/parser.ts` `ActionAssembler` turns the stream into ordered `Action`s (one `speak` action per whole `[SPEAK]` block). `useVoiceInterview`'s **action runner** synthesizes a line's audio fully (§17.6), reveals the text, plays the buffer, and types the code that follows it over the audio's **measured** duration — so speech and code stay locked (no drift) and nothing stutters. Helpers in `lib/coding/playback.ts` (`ActionQueue`, `tokenizeCode`).
- **In-place edits (no full rewrite):** the editor is **no longer wiped each turn**; the current contents are sent as context and follow-ups patch them. `lib/coding/edits.ts` `locate()` finds the snippet (exact, then indentation-tolerant line match) and the runner animates the replacement in place; `deleteSnippet()` removes a snippet. If a snippet isn't found the new code is appended (never lost). Editor contents persist to `interviewiq_code_{id}`. Monaco scrolls to the changed region (mid-file edits visible).
- **Barge-in / interrupt:** a per-turn `runIdRef` stops the runner; an in-flight typing loop dumps the rest of the current line so the editor stays coherent, then bails.

### 17.13 Always-on voice + mute (replaces Start/Pause/Resume)
The interview connects automatically when the candidate loads — there is no Start/Pause/Resume flow. `useVoiceInterview` auto-calls `start()` once `candidate` is set (guarded on `status==='idle'`).
- **Mute** is the only control: `setMuted/toggleMute` → `mic.setMuted()` disables the audio track (RMS → 0, no barge-in, no turns) while the recorder keeps emitting **silent** frames so the Deepgram STT socket doesn't time out. The session, AudioContext, STT/TTS sockets all stay live; the candidate can still speak while you're muted. Muting also clears any half-captured turn so it isn't committed.
- **Autoplay unlock:** auto-start can leave the AudioContext `suspended` (no user gesture yet). `start()` installs a one-time `pointerdown`/`keydown` listener that resumes it, so the candidate's voice becomes audible on the interviewer's first interaction (mic capture itself needs no gesture). Listeners are removed in `teardown`.
- **Permission/gesture fallback:** if `start()` errors (denied mic, etc.) the UI shows an **"Enable microphone"** button that calls `start()` from a real click. `pause`/`paused` are gone from the hook API; `stop()` still tears down on unmount / End Interview.
